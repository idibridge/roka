...
(function(){
  ...
  */
  ...
  function applyLang(lang) {
    ...
    /* Fix select elements — force visual refresh after option text change */
 ...
 *  4. Вставь токен и chat_id ниже
 */
var TG_BOT_TOKEN = '8051623292:AAEA0ETZhdzpiyGVVnI1s-9_jflzxyxBAuE';
...
function handleContactForm(e) {
  ...
  var company = inputs[1] ? inputs[1].value.trim() : '';
  ...
  var message = inputs[5] ? inputs[5].value.trim() : '';
  ...
  if (TG_BOT_TOKEN === 'ВСТАВЬ_ТОКЕН_СЮДА') {
    /* Токен не настроен — показываем успех для тестирования */
    console.warn('ROKA: Telegram bot token not configured. Form data:', { name: name, company: company, phone: phone, email: email, type: type, message: message });
  ...
  .catch(function() {
    alert('Ошибка сети. Попробуйте связаться через Telegram или WhatsApp.');
    if (btn) { btn.disabled = false; btn.style.opacity = '1'; }
  });
